...
function GroupSet() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log(
          `${coSn}-회사SN으로 과정리스트 불러오기API 실행 >>>>>>>>>>>>>>>>>>`
        );
        setLoading(true);
        const res = await hortlistByCpSn(Number(coSn));
        console.log(res);
        console.log(
          `${res.data}-회사SN으로 과정리스트 불러오기API 응답 <<<<<<<<<<<<<<<<<<<`
        );
  ...
  // 필터 배열
  useEffect(() => {
    const list = (hortlist || []).map((h) => ({
      cohortSn: h?.cohortSn,
      cohortNm: h?.cohortNm || '이름 없음',
    }));
    setFilterArr(list);
  }, [hortlist]);

  // 과정 리스트 불러온 뒤 기본 선택값 지정
  useEffect(() => {
    if (hortlist && hortlist.length > 0 && !selectedCohortSn) {
      const first = hortlist[0];
      setSelectedCohortSn(first.cohortSn);
      setSelected(0);
      console.log('[GroupSet] 기본 선택된 기수:', first);
    }
  }, [hortlist]);
    ...
    <div className="boardPage">
      ...
      <div className="filterList">
        <FilterList
          ...
          setCohortSn={setSelectedCohortSn}
      ...
      <div className="mainCont_Lms_Row">
        ...
        <div className="main_R" style={{ flex: '1', gap: '16px' }}>
          ...
          <div className="dashBoardModule" style={{ overflow: 'hidden' }}>
            <TodayAttendList cohortSn={selectedCohortSn} />
          </div>
        </div>
      </div>
    </div>
